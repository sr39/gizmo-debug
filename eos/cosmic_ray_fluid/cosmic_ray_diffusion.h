...
for(k_CRegy=0;k_CRegy<N_CR_PARTICLE_BINS;k_CRegy++)
{
    ...
    if(((kappa_i>MIN_REAL_NUMBER)||(kappa_j>MIN_REAL_NUMBER))&&(local.Mass>0)&&(P[j].Mass>0)&&(dt_hydrostep>MIN_REAL_NUMBER)&&(Face_Area_Norm>MIN_REAL_NUMBER))
    {
        ...
        if(fabs(diffusion_wt) > 0)
        {
            ...
            double CR_egy_i = local.CosmicRayPressure[k_CRegy]*V_i / (GAMMA_COSMICRAY(k_CRegy)-1.); // (E_cr = Volume * (Pressure/GAMMA_COSMICRAY_MINUS1)) - this is physical units //
            double CR_egy_j = CosmicRayPressure_j[k_CRegy]*V_j / (GAMMA_COSMICRAY(k_CRegy)-1.);
        ...
        double f_direct = -Face_Area_Norm * c_hll * (d_scalar/(GAMMA_COSMICRAY(k_CRegy)-1.)) * renormerFAC; // simple HLL term for frame moving at 1/2 inter-particle velocity: here not limited //
        if(dt_hydrostep > 0)
        {
        ...
        if(cmag != 0)
        {
            ...
            double sVi = scalar_i*V_i_phys/(GAMMA_COSMICRAY(k_CRegy)-1.), sVj = scalar_j*V_j_phys/(GAMMA_COSMICRAY(k_CRegy)-1.); // physical units

...
#GALSF_FB_FIRE_AGE_TRACERS      #- add explicit tracking of tracer species for arbitrary stellar yields as a function of age from different stellar populations, developed by Andrew Emerick (2020, in prep). enabled by default with the FIRE defaults
############################################################################################################################
...
#COSMIC_RAYS_ALFVEN=(500.)      #- extends the M1 formalism above following Thomas+Pfrommer 18 to explicitly evolve gyro-resonant Alfven-wave packets with self-confinement motivated CR scattering rates; value here is the reduced speed of light (maximum free-streaming speed) in code units. requires MAGNETIC and COOLING for detailed MHD and ionization+thermal states. cite Hopkins et al. arXiv:2002.06211 for numerical implementation into various solvers here, Thomas+Pfrommer 18 for equations for gyro-resonant packets
##------
...
#COSMIC_RAYS_EVOLVE_SPECTRUM    #- follow a spectrally-resolved CR population of e- and p from ~MeV-TeV (and other species if extended network is enabled), including injection and adiabatic+hadronic/catastrophic/pionic/fragmentation+inverse compton+ionization+coulomb+bremstrahhlung+gyroresonant/streaming+synchrotron+annihilation+radioactive losses and (optionally) re-acceleration. cite Hopkins et al. 2021 (in prep, methods in testing until published) for implementation+electrons+loss/gain terms and Girichidis+ 2020MNRAS.491..993G for the fundamental spectral bin-to-bin method
#COSMIC_RAYS_DIFFUSIVE_REACCELERATION #- models diffusive re-acceleration with the usual assumption that CR scattering/diffusion is dominated by an undamped isotropic extrinsic turbulent cascade (not likely valid below ~TeV), following e.g. Drury+Strong 2017A&A...597A.117D; requires COSMIC_RAYS_EVOLVE_SPECTRUM.
#COSMIC_RAYS_EVOLVE_SPECTRUM_EXTENDED_NETWORK #- extend the resolved CR spectral models to a primary+secondary network with e-,e+,p,anti-p,B,C/N/O,stable (7-9)Be, unstable 10Be, following various reaction networks. requires COSMIC_RAYS_EVOLVE_SPECTRUM. cite Hopkins et al. 2021 (in prep; module still in testing, not for widespread use until methods published)
##------

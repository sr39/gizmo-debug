...
void hydro_final_operations_and_cleanup(void)
{
    ...
    for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
    {
        if(P[i].Type == 0 && P[i].Mass > 0)
        {
...
#if defined(COSMIC_RAYS_ALT_RSOL_FORM) && defined(COSMIC_RAYS_M1) && defined(MAGNETIC) // only makes sense to include parallel correction below if all these terms enabled //
            ...
            if(Bmag>0) {Bmag=sqrt(Bmag); for(m=0;m<3;m++) {bhat[k] /= Bmag;}} // make dimmanetionless
            ...
            if(Bmag>0) {for(k=0;k<N_CR_PARTICLE_BINS;k++) {
                int m; double grad_P_dot_B=0, F_dot_B=0, h_cr=GAMMA_COSMICRAY * SphP[i].CosmicRayEnergyPred[k] * vol_inv, vA_k=vA_eff, fcorr[3]={0};
                for(m=0;m<3;m++) {grad_P_dot_B += bhat[m] * SphP[i].Gradients.CosmicRayPressure[k][m] * (All.cf_a3inv/All.cf_atime); F_dot_B += bhat[m] * SphP[i].CosmicRayFluxPred[k][m] * vol_inv;}

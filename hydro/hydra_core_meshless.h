...
#if (SLOPE_LIMITER_TOLERANCE != 2) && !((defined(HYDRO_FACE_AREA_LIMITER) || !defined(PROTECT_FROZEN_FIRE)) && (HYDRO_FIX_MESH_MOTION >= 5)) || (MHD_CONSTRAINED_GRADIENT>=1 && defined(SINGLE_STAR_SINK_DYNAMICS)) // unless using the most aggressive reconstruction, we will limit face-area disparity here //

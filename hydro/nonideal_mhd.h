...
if((local.Mass > 0) && (P[j].Mass > 0))
{
    ...
    if(eta_max > 0)
    {
        ...
        J_current[0] = (kernel.dp[1]*d_scalar[2] - kernel.dp[2]*d_scalar[1]) * rinv2;
        J_current[1] = (kernel.dp[2]*d_scalar[0] - kernel.dp[0]*d_scalar[2]) * rinv2;
        J_current[2] = (kernel.dp[0]*d_scalar[1] - kernel.dp[1]*d_scalar[0]) * rinv2;
        ...
        double J_direct[3];
        J_direct[0] = (kernel.dp[1]*d_scalar[2] - kernel.dp[2]*d_scalar[1]) * rinv2;
        J_direct[1] = (kernel.dp[2]*d_scalar[0] - kernel.dp[0]*d_scalar[2]) * rinv2;
        J_direct[2] = (kernel.dp[0]*d_scalar[1] - kernel.dp[1]*d_scalar[0]) * rinv2;
#endif
        // calculate the actual fluxes //
        double b_flux[3], JcrossB[3], JcrossBcrossB[3], BdotA=0, JdotA=0, JdotA_direct=0;
        for(k=0;k<3;k++) {b_flux[k] = eta_ohmic * J_current[k];} // ohmic ~ J
        ...
        if(eta_hall>0)
        {
            for(k=0;k<3;k++) {JdotA+=J_current[k]*Face_Area_Vec[k]; JdotA_direct+=J_direct[k]*Face_Area_Vec[k]; BdotA+=bhat[k]*Face_Area_Vec[k];}
            for(k=0;k<3;k++) {b_flux[k] += hallsign * eta_hall * JcrossB[k];} // hall ~ (JxB)
        }
        ...
        double eta_direct_sum = eta_ohmic_0 + eta_ad_0 + eta_hall_0;
        ...
        double b_flux_direct[3], JcrossB_direct[3], JcrossBcrossB_direct[3], db_direct[3], db_direct_0[3];
        for(k=0;k<3;k++) {b_flux_direct[k] = eta_ohmic_0 * J_direct[k];}
        ...
        for(k=0;k<3;k++) {db_direct_0[k]=db_direct[k];}
        if(eta_hall>0)
        {
            for(k=0;k<3;k++)
            {
                double q = hallsign * eta_hall_0 * (BdotA*J_direct[k] - JdotA_direct*bhat[k]);
                db_direct_0[k] += q;
                double corrfac = -hallsign * eta_hall * (BdotA*J_current[k] - JdotA*bhat[k]);
                if(local.BPred[k]<BPred_j[k]) {if(q>0) {db_direct[k]+=q;} else {db_direct[k]+=corrfac;}} else {if(q<0) {db_direct[k]+=q;} else {db_direct[k]+=corrfac;}}
            }
        }
        // add this numerical diffusivity to the higher-order fluxes, and then flux-limit the result based on the upper limit below //
        double bmag_ij=0; for(k=0;k<3;k++) {bmag_ij += (local.BPred[k]-BPred_j[k]) * (local.BPred[k]-BPred_j[k]);}
        double BFDirmag = db_direct_0[0]*db_direct_0[0]+db_direct_0[1]*db_direct_0[1]+db_direct_0[2]*db_direct_0[2];
        double b_flux_test = eta_max * sqrt(bmag_ij) * rinv / (eta_direct_sum * sqrt(1.e-37 + BFDirmag));
        for(k=0;k<3;k++)
        {
            bflux_from_nonideal_effects[k] += db_direct[k];
            bflux_from_nonideal_effects[k] = MINMOD(bflux_from_nonideal_effects[k] , b_flux_test * db_direct_0[k]);
        }
#endif

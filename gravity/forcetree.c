...
#ifdef RT_OTVET
        if(valid_gas_particle_for_rt) {int k_et; for(k_et=0;k_et<6;k_et++) {SphP[target].ET[k_et] = ET[k_et];}}
#endif
#ifdef GALSF_FB_LOCAL_UV_HEATING
...
#ifdef RT_OTVET
        int k_et; for(k_et=0;k_et<6;k_et++) {GravDataResult[target].ET[k_et] = ET[k_et];}
#endif
#ifdef GALSF_FB_LOCAL_UV_HEATING

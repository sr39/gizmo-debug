                                 ...
                                 double *mu_guess)
{
    ...
    *mu_guess=Get_Gas_Mean_Molecular_Weight_mu(pow(10.,logT), rho, nH0_guess, ne_guess, sqrt(shieldfac)*(gJH0/2.29e-10), target);
    if(target >= 0) /* if this is a cell, update some of its thermodynamic stored quantities */
    {
        ...
        SphP[target].MolecularMassFraction = Get_Gas_Molecular_Mass_Fraction(target, pow(10.,logT), DMIN(DMAX(nH0,0),1), sqrt(shieldfac)*(gJH0/2.29e-10) , 1);
#endif
    }
...
double CoolingRate(double logT, double rho, double n_elec_guess, int target)
{
    ...
    if(logT < Tmax)
    {
        ...
        if(logT <= 5.3)
        {
            ...
            double f_not_strongly_ionized = DMIN(DMAX(nH0,0),1); // fraction not being ionized or otherwise exposed to -very- strong radiation which would suppress cooling even from e.g. C+ [hence 1-shieldfac appearing, not just nH0]
...
#endif
#ifdef OUTPUT_MOLECULAR_FRACTION
    if(target>0) {SphP[target].MolecularMassFraction = Get_Gas_Molecular_Mass_Fraction(target, T, DMIN(DMAX(nH0,0),1), sqrt(shieldfac)*(gJH0/2.29e-10) , 1);;}
#endif
    ...
    int i; for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
    {
        if(P[i].Type==0)
        {
            if((SphP[i].Rad_Flux_UV>0) && (PPP[i].Hsml>0) && (SphP[i].Density>0) && (P[i].Mass>0) && (All.Time>0))
            {
                ...
                SphP[i].Rad_Flux_UV *= 0.01 + 0.99/(1.0 + 0.8*tau_nuv + 0.85*tau_nuv*tau_nuv); // attenuate (for clumpy medium with 1% scattering)

...
double H2_rovibrational_cooling(double T, double xH2, double xH, double xHII, double xHe, double xe, double nH_tot)
{
  ...
  double dT;
  if (T <= 100.0)
    LowDens_cool_rate = pow(10.0, -16.818342 + 37.383713 * log10(T3) + 58.145166 * pow(log10(T3), 2) + 48.656103 * pow(log10(T3), 3) + 20.159831 * pow(log10(T3), 4) + 3.847961 * pow(log10(T3), 5)) * xH2 * xH;
  else if (T <= 1000.0)
    LowDens_cool_rate = pow(10.0, -24.311209 + 3.5692468 * log10(T3) - 11.33286 * pow(log10(T3), 2) - 27.850082 * pow(log10(T3), 3) - 21.328264 * pow(log10(T3), 4) - 4.2519023 * pow(log10(T3), 5)) * xH2 * xH;
  else
    LowDens_cool_rate = pow(10.0, -24.311209 + 4.6450521 * log10(T3) - 3.7209846 * pow(log10(T3), 2) + 5.9369081 * pow(log10(T3), 3) - 5.5108047 * pow(log10(T3), 4) + 1.5538288 * pow(log10(T3), 5)) * xH2 * xH;
  LowDens_cool_rate += pow(10.0, -23.962112 + 2.0943374 * log10(T3) - 0.77151436 * pow(log10(T3), 2) + 0.43693353 * pow(log10(T3), 3) - 0.14913216 * pow(log10(T3), 4) - 0.033638326 * pow(log10(T3), 5)) * xH2 * xH2;
  LowDens_cool_rate += pow(10.0, -23.689237 + 2.1892372 * log10(T3) - 0.81520438 * pow(log10(T3), 2) + 0.29036281 * pow(log10(T3), 3) - 0.16596184 * pow(log10(T3), 4) + 0.19191375 * pow(log10(T3), 5)) * xH2 * xHe;
  LowDens_cool_rate += pow(10.0, -21.716699 + 1.3865783 * log10(T3) - 0.37915285 * pow(log10(T3), 2) + 0.11453688 * pow(log10(T3), 3) - 0.23214154 * pow(log10(T3), 4) + 0.058538864 * pow(log10(T3), 5)) * xH2 * xHII;
  if (T <= 200.0)
    LowDens_cool_rate += pow(10.0, -34.286155 - 48.537163 * log10(T3) - 77.121176 * pow(log10(T3), 2) - 51.352459 * pow(log10(T3), 3) - 15.16916 * pow(log10(T3), 4) - 0.98120322 * pow(log10(T3), 5)) * xH2 * xe;
  else
    LowDens_cool_rate += pow(10.0, -22.190316 + 1.5728955 * log10(T3) - 0.213351 * pow(log10(T3), 2) + 0.96149759 * pow(log10(T3), 3) - 0.91023195 * pow(log10(T3), 4) + 0.13749749 * pow(log10(T3), 5)) * xH2 * xe;
  
  /* Obtain the LTE rate from the molecular 
   * cooling tables. */
  get_index_1d_mydbl(chimesRateTables.mol_cooling_table_H2_lte_temperatures, chimesRateTables.mol_cooling_table_dimensions[10], log10(T), &T_index, &dT);
  LTE_cool_rate = pow(10.0, interpol_1d_mydbl(chimesRateTables.mol_cooling_table_H2_lte, T_index, dT)) * (xH2 / nH_tot);   /* lambda / nH_tot ^ 2 (in units erg cm^3 s^-1) */
...
double CO_rotational_cooling(double T, double N, double xm, double xH2, double xHI, double xe, double nH, struct globalVariables *myGlobalVars)
{
  int i, j;
  double dT, dN;
   ...
   * spaced, (T, N) grid. 
   */
  get_index_1d_irregular(chimesRateTables.mol_cooling_table_CO_rot_T, chimesRateTables.mol_cooling_table_dimensions[0], log10(T), &i, &dT);
...
double CO_vibrational_cooling(double T, double N, double xm, double xH2, double xHI, double xe, double nH, struct globalVariables *myGlobalVars)
{
  int i, j;
  double dT, dN;
  ...
  get_index_1d_irregular(chimesRateTables.mol_cooling_table_CO_vib_T, chimesRateTables.mol_cooling_table_dimensions[2], log10(T), &i, &dT);
...
double H2O_rotational_cooling(double T, double N, double xm, double xH2, double xHI, double xe, double nH, struct globalVariables *myGlobalVars)
{
  int i, j;
  double dT, dN;
  double Lm, L0, Llte, nhalf, a, neff;
  ...
  if (T >= 100.0)
    {
      /* Obtain the indices for T and N. */	
      get_index_1d_irregular(chimesRateTables.mol_cooling_table_H2O_rot_hiT_T, chimesRateTables.mol_cooling_table_dimensions[4], log10(T), &i, &dT);
      get_index_1d_mydbl(chimesRateTables.mol_cooling_table_H2O_rot_hiT_N, chimesRateTables.mol_cooling_table_dimensions[5], log10(N), &j, &dN);
      ...
      neff = nH * (xH2 + 10.0 * xHI + pow(10.0, (-8.02 + (15.749 / pow(T, 1.0 / 6.0)) - (47.137 / pow(T, 1.0 / 3.0)) + (76.648 / pow(T, 0.5)) - (60.191 / pow(T, 2.0 / 3.0)))) * xe / (7.4e-12 * pow(T, 0.5)));
    ...
    {		
      /* Obtain the indices for T and N. */	
      get_index_1d_irregular(chimesRateTables.mol_cooling_table_H2O_rot_lowT_T, chimesRateTables.mol_cooling_table_dimensions[6], log10(T), &i, &dT);
      get_index_1d_mydbl(chimesRateTables.mol_cooling_table_H2O_rot_lowT_N, chimesRateTables.mol_cooling_table_dimensions[7], log10(N), &j, &dN);
...
double H2O_vibrational_cooling(double T, double N, double xm, double xH2, double xHI, double xe, double nH, struct globalVariables *myGlobalVars)
{
  ...
  /* Obtain the indices for T and N. */
  get_index_1d_irregular(chimesRateTables.mol_cooling_table_H2O_vib_T, chimesRateTables.mol_cooling_table_dimensions[8], log10(T), &i, &dT);
  get_index_1d_mydbl(chimesRateTables.mol_cooling_table_H2O_vib_N, chimesRateTables.mol_cooling_table_dimensions[9], log10(N), &j, &dN);
...
double calculate_total_cooling_rate(struct gasVariables *myGasVars, struct globalVariables *myGlobalVars, double HI_column_density, double HeI_column_density, double HeII_column_density, double H2_column_density, double CO_column_density, double H2O_column_density, double OH_column_density, double extinction, struct All_rate_variables_structure *this_all_rates)
{
  ...
  if (log10(myGasVars->temperature) < chimesRateTables.nei_cooling_temperature[chimesRateTables.nei_cooling_table_dimensions[0] - 1])
    {
      get_index_1d_mydbl(chimesRateTables.nei_cooling_temperature, chimesRateTables.nei_cooling_table_dimensions[0], log10(myGasVars->temperature), &m, &dT_2);
      get_index_1d_mydbl(chimesRateTables.nei_cooling_HIAbundance, chimesRateTables.nei_cooling_table_dimensions[1], log10(max(myGasVars->abundances[myGlobalVars->speciesIndices[HI]] * myGasVars->nH_tot, 1.0e-100)), &j, &dnHI);
      get_index_1d_mydbl(chimesRateTables.nei_cooling_ElectronAbundance, chimesRateTables.nei_cooling_table_dimensions[2], log10(max(myGasVars->abundances[myGlobalVars->speciesIndices[elec]] * myGasVars->nH_tot, 1.0e-100)), &k, &dne);
      get_index_1d_mydbl(chimesRateTables.nei_cooling_HIIAbundance, chimesRateTables.nei_cooling_table_dimensions[3], log10(max(myGasVars->abundances[myGlobalVars->speciesIndices[HII]] * myGasVars->nH_tot, 1.0e-100)), &p, &dnHII);
    }
  if (log10(myGasVars->temperature) < chimesRateTables.chianti_cooling_temperature[chimesRateTables.chianti_cooling_table_dimensions[0] - 1])
    {
      get_index_1d_mydbl(chimesRateTables.chianti_cooling_temperature, chimesRateTables.chianti_cooling_table_dimensions[0], log10(myGasVars->temperature), &chianti_T_index, &chianti_dT);
      get_index_1d_mydbl(chimesRateTables.chianti_cooling_ElectronDensity, chimesRateTables.chianti_cooling_table_dimensions[1], log10(max(myGasVars->abundances[myGlobalVars->speciesIndices[elec]] * myGasVars->nH_tot, 1.0e-100)), &chianti_ne_index, &chianti_dne);
    }
  ...
  for (ns = 0; ns < chimesRateTables.NonEqIon->N_Elements; ns++)
	...
	{
	  if (log10(myGasVars->temperature) >= chimesRateTables.nei_cooling_temperature[chimesRateTables.nei_cooling_table_dimensions[0] - 1] || ((chimesRateTables.NonEqIon->IonIndexBegin[ns] + i) != CI && (chimesRateTables.NonEqIon->IonIndexBegin[ns] + i) != OI))
	    {
	      if (log10(myGasVars->temperature) >= chimesRateTables.chianti_cooling_temperature[chimesRateTables.chianti_cooling_table_dimensions[0] - 1] || ((chimesRateTables.NonEqIon->IonIndexBegin[ns] + i) != CII && (chimesRateTables.NonEqIon->IonIndexBegin[ns] + i) != NII && (chimesRateTables.NonEqIon->IonIndexBegin[ns] + i) != SiII && (chimesRateTables.NonEqIon->IonIndexBegin[ns] + i) != FeII))
		total_cooling += pow(10.0, interpol_1d_mydbl(chimesRateTables.NonEqIon->NonEqRates[ns].cool[i], T_index, dT)) * myGasVars->abundances[myGlobalVars->speciesIndices[chimesRateTables.NonEqIon->IonIndexBegin[ns] + i]] * myGasVars->abundances[myGlobalVars->speciesIndices[elec]];
	    }
	  if (i != (chimesRateTables.NonEqIon->N_Ions[ns] - 1))  /* The final ion cannot be photoionised further */
	      ...
	      for (l = 0; l < myGlobalVars->N_spectra; l++) 
		{
		  ...
		  if (chimesRateTables.NonEqIon->NonEqRates[ns].E_thresh[i] >= 13.6)
		      ...
		      else
			{
			  if (chimesRateTables.NonEqIon->NonEqRates[ns].E_thresh[i] < 15.4)
			    {
			      S1 = pow(10.0, interpol_1d_fltdbl(chimesRateTables.NonEqIon->NonEqRates[ns].shieldFactor1D[l][0][i], NHI_index, dNHI));
			      S2 = pow(10.0, interpol_2d_fltdbl(chimesRateTables.NonEqIon->NonEqRates[ns].shieldFactor2D[l][0][i], NH_eff_index, NHeI_index, dNH_eff, dNHeI));
			      S3 = pow(10.0, interpol_2d_fltdbl(chimesRateTables.NonEqIon->NonEqRates[ns].shieldFactor2D[l][1][i], NH_eff_index, NHe_eff_index, dNH_eff, dNHe_eff));
			    }
		  ...
		  if (chimesRateTables.NonEqIon->NonEqRates[ns].E_thresh[i] < 13.6 || (HI_column_density == 0.0 && H2_column_density == 0.0 && HeI_column_density == 0.0 && HeII_column_density == 0))
		    epsilon = chimesRateTables.NonEqIon->NonEqRates[ns].epsilon[l][i];
		  else
		    {
		      if (chimesRateTables.NonEqIon->NonEqRates[ns].E_thresh[i] < 15.4)
			{
			  E1 = pow(10.0, interpol_1d_fltdbl(chimesRateTables.NonEqIon->NonEqRates[ns].shieldFactor1D[l][1][i], NHI_index, dNHI));
			  E4 = pow(10.0, interpol_1d_fltdbl(chimesRateTables.NonEqIon->NonEqRates[ns].shieldFactor1D[l][2][i], NHI_index, dNHI));
			  E2 = pow(10.0, interpol_2d_fltdbl(chimesRateTables.NonEqIon->NonEqRates[ns].shieldFactor2D[l][2][i], NH_eff_index, NHeI_index, dNH_eff, dNHeI));
			  E5 = pow(10.0, interpol_2d_fltdbl(chimesRateTables.NonEqIon->NonEqRates[ns].shieldFactor2D[l][4][i], NH_eff_index, NHeI_index, dNH_eff, dNHeI));
			  E3 = pow(10.0, interpol_2d_fltdbl(chimesRateTables.NonEqIon->NonEqRates[ns].shieldFactor2D[l][3][i], NH_eff_index, NHe_eff_index, dNH_eff, dNHe_eff));
			  E6 = pow(10.0, interpol_2d_fltdbl(chimesRateTables.NonEqIon->NonEqRates[ns].shieldFactor2D[l][5][i], NH_eff_index, NHe_eff_index, dNH_eff, dNHe_eff));
			}
		      else if (chimesRateTables.NonEqIon->NonEqRates[ns].E_thresh[i] < 54.42)
			{
			  E1 = 0.0;
		  ...
		  total_cooling += photoheating(myGasVars->abundances[myGlobalVars->speciesIndices[chimesRateTables.NonEqIon->IonIndexBegin[ns] + i]], photoion_rate, epsilon, myGasVars->nH_tot);
		}
	      ...
	      if (chimesRateTables.NonEqIon->IonIndexBegin[ns] == HI || (chimesRateTables.NonEqIon->IonIndexBegin[ns] + i) == HeI)
		{
		  ...
		  total_cooling += cosmic_ray_heating(myGasVars->abundances[myGlobalVars->speciesIndices[chimesRateTables.NonEqIon->IonIndexBegin[ns] + i]], this_all_rates->BensRates[ns].cosmicRays[i], myGasVars->nH_tot) / (1.0 + cr_secondary_ionisation(myGasVars->abundances[myGlobalVars->speciesIndices[HII]], chimesRateTables.NonEqIon->IonIndexBegin[ns]));
		}
	      else
		total_cooling += cosmic_ray_heating(myGasVars->abundances[myGlobalVars->speciesIndices[chimesRateTables.NonEqIon->IonIndexBegin[ns] + i]], this_all_rates->BensRates[ns].cosmicRays[i], myGasVars->nH_tot);
	    }
	}
      ...
      if (chimesRateTables.NonEqIon->IonIndexBegin[ns] == HI)
	{
	  ...
	  for (i = 1; i < 3; i++)
	      ...
	      for (l = 0; l < myGlobalVars->N_spectra; l++) 
		{
		  ...
		  if (i == 1)
		    total_cooling += photoheating(myGasVars->abundances[myGlobalVars->speciesIndices[Hm]], photoion_rate, epsilon, myGasVars->nH_tot);
		  else
		    total_cooling += photoheating(myGasVars->abundances[myGlobalVars->speciesIndices[H2]], photoion_rate, epsilon, myGasVars->nH_tot);
		}
	    }
	}
    }
  ...
  if (log10(myGasVars->temperature) < chimesRateTables.nei_cooling_temperature[chimesRateTables.nei_cooling_table_dimensions[0] - 1])
    {
      ...
      total_cooling += pow(10, interpol_4d_mydbl(chimesRateTables.nei_cooling_CI, m, j, k, p, dT_2, dnHI, dne, dnHII)) * myGasVars->abundances[myGlobalVars->speciesIndices[CI]] / myGasVars->nH_tot;
      total_cooling += pow(10, interpol_4d_mydbl(chimesRateTables.nei_cooling_OI, m, j, k, p, dT_2, dnHI, dne, dnHII)) * myGasVars->abundances[myGlobalVars->speciesIndices[OI]] / myGasVars->nH_tot;
    }
  ...
  if (log10(myGasVars->temperature) < chimesRateTables.chianti_cooling_temperature[chimesRateTables.chianti_cooling_table_dimensions[0] - 1])
    {
      total_cooling += pow(10, interpol_2d_mydbl(chimesRateTables.chianti_cooling_CII, chianti_T_index, chianti_ne_index, chianti_dT, chianti_dne)) * myGasVars->abundances[myGlobalVars->speciesIndices[CII]] * myGasVars->abundances[myGlobalVars->speciesIndices[elec]];
      total_cooling += pow(10, interpol_2d_mydbl(chimesRateTables.chianti_cooling_NII, chianti_T_index, chianti_ne_index, chianti_dT, chianti_dne)) * myGasVars->abundances[myGlobalVars->speciesIndices[NII]] * myGasVars->abundances[myGlobalVars->speciesIndices[elec]];
      total_cooling += pow(10, interpol_2d_mydbl(chimesRateTables.chianti_cooling_SiII, chianti_T_index, chianti_ne_index, chianti_dT, chianti_dne)) * myGasVars->abundances[myGlobalVars->speciesIndices[SiII]] * myGasVars->abundances[myGlobalVars->speciesIndices[elec]];
      total_cooling += pow(10, interpol_2d_mydbl(chimesRateTables.chianti_cooling_FeII, chianti_T_index, chianti_ne_index, chianti_dT, chianti_dne)) * myGasVars->abundances[myGlobalVars->speciesIndices[FeII]] * myGasVars->abundances[myGlobalVars->speciesIndices[elec]];
    }
  ...
  if (myGasVars->temperature <= myGlobalVars->T_mol)
    {
      /* Cooling from H2 */
      total_cooling += H2_rovibrational_cooling(myGasVars->temperature, myGasVars->abundances[myGlobalVars->speciesIndices[H2]], myGasVars->abundances[myGlobalVars->speciesIndices[HI]], myGasVars->abundances[myGlobalVars->speciesIndices[HII]], myGasVars->abundances[myGlobalVars->speciesIndices[HeI]], myGasVars->abundances[myGlobalVars->speciesIndices[elec]], myGasVars->nH_tot); /*GA08 */
      total_cooling += 7.2e-12 * (k9(myGasVars->temperature, myGasVars->nH_tot, myGasVars->abundances[myGlobalVars->speciesIndices[HI]], myGasVars->abundances[myGlobalVars->speciesIndices[H2]], myGasVars->abundances[myGlobalVars->speciesIndices[HeI]]) * myGasVars->abundances[myGlobalVars->speciesIndices[HI]] + (this_all_rates->rate_10 / myGasVars->nH_tot) * myGasVars->abundances[myGlobalVars->speciesIndices[H2]]) * myGasVars->abundances[myGlobalVars->speciesIndices[H2]]; /* H2 collis diss */
      total_cooling -= 6.4e-13 * this_all_rates->rate_53 * myGasVars->abundances[myGlobalVars->speciesIndices[H2]] / myGasVars->nH_tot; /* H2 photodiss */
      n_cr = H2_crit_density(myGasVars->temperature, myGasVars->abundances[myGlobalVars->speciesIndices[HI]], myGasVars->abundances[myGlobalVars->speciesIndices[H2]]);
      n = calculate_total_number_density(myGasVars->abundances, myGasVars->nH_tot, myGlobalVars); 
      total_cooling -= 2.7e-11 * this_all_rates->rate_53 * myGasVars->abundances[myGlobalVars->speciesIndices[H2]] * (n / (n + n_cr)) / myGasVars->nH_tot; /* H2 UV pumping */
      total_cooling -= (2.93e-12 * (this_all_rates->rate_2 / myGasVars->nH_tot) * myGasVars->abundances[myGlobalVars->speciesIndices[Hm]] + 5.65e-12 * (this_all_rates->rate_4 / myGasVars->nH_tot) * myGasVars->abundances[myGlobalVars->speciesIndices[H2p]]) * myGasVars->abundances[myGlobalVars->speciesIndices[HI]] * (n / (n + n_cr)); /* Gas-phase H2 formation */
      total_cooling -= 7.16e-12 * (this_all_rates->rate_60 / myGasVars->nH_tot) * myGasVars->abundances[myGlobalVars->speciesIndices[HI]] * n * (n / (n + n_cr)) / myGasVars->nH_tot; /* Dust H2 formation */
      total_cooling += cosmic_ray_heating(myGasVars->abundances[myGlobalVars->speciesIndices[H2]], 2.0 * myGasVars->cr_rate, myGasVars->nH_tot);
      ...
      if (myGlobalVars->element_included[0] == 1 && myGlobalVars->element_included[2] == 1)
	{
	  ...
	  total_cooling += CO_vibrational_cooling(myGasVars->temperature, N_eff, myGasVars->abundances[myGlobalVars->speciesIndices[CO]], myGasVars->abundances[myGlobalVars->speciesIndices[H2]], myGasVars->abundances[myGlobalVars->speciesIndices[HI]], myGasVars->abundances[myGlobalVars->speciesIndices[elec]], myGasVars->nH_tot, myGlobalVars);
	}
	...
	dust_G += myGasVars->isotropic_photon_density[l] * C_LIGHT * myGasVars->dust_G_parameter[l]; 
      ...
      total_cooling += grain_surface_recombination(myGasVars->temperature, myGasVars->metallicity, myGasVars->abundances[myGlobalVars->speciesIndices[elec]] * myGasVars->nH_tot, myGasVars->nH_tot, dust_G, extinction);
    }
...
void do_equilibrium_cooling(void *user_data)
{
  ...
  if (u - u_old - LambdaNet * dt < 0.0) /* heating */
      ...
      while (u_upper - u_old - LambdaNet * dt < 0.0 && i < maxIter)
	{
      ...
      if(i == maxIter) PRINT_WARNING("Problem with eqm cooling finding the upper bound");
    }
  else /* cooling */ 
    {
      u_upper *= sqrt(1.2);
      ...
      else
	{
	  ...
	  if (i == maxIter) PRINT_WARNING("Problem with eqm cooling finding the lower bound");
	}
  ...
  if (i >= maxIter) PRINT_WARNING("eqm cooling failed to converge");

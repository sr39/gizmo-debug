...
void do_equilibrium_cooling(void *user_data)
{
  ...
  if (u - u_old - LambdaNet * dt < 0.0) /* heating */
      ...
      while (u_upper - u_old - LambdaNet * dt < 0.0 && i < maxIter)
	{
      ...
      if(i == maxIter) printf("Problem with eqm cooling finding the upper bound");
    }
  else /* cooling */ 
    {
      u_upper *= sqrt(1.2);
      ...
      else
	{
	  ...
	  if (i == maxIter) printf("Problem with eqm cooling finding the lower bound");
	}
  ...
  if (i >= maxIter) printf("eqm cooling failed to converge");

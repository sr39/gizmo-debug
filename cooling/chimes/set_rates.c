...
void update_rates(struct gasVariables *myGasVars, struct globalVariables *myGlobalVars, double HI_column_density, double H2_column_density, double HeI_column_density, double HeII_column_density, double CO_column_density, double extinction, struct All_rate_variables_structure *this_all_rates)
{
  ...
  this_all_rates->rate_53 = R53(H2_column_density, T, myGasVars->isotropic_photon_density, extinction, myGasVars->doppler_broad, myGlobalVars, myGasVars);
  ...
  for (i = 0; i < chimesRateTables.NonEqIon->N_Elements; i++)
	...
	this_all_rates->BensRates[i].cosmicRays[0] = chimesRateTables.NonEqIon->NonEqRates[i].cosmicRays[0] * cr_rate * (1.0 + cr_secondary_ionisation(xHII, chimesRateTables.NonEqIon->IonIndexBegin[i]));
	  ...
	  for (l = 0; l < myGlobalVars->N_spectra; l++) 
	      ...
	      if (chimesRateTables.NonEqIon->NonEqRates[i].E_thresh[j] >= 13.6)
		{
		  if (HI_column_density == 0.0 && H2_column_density == 0.0 && HeI_column_density == 0.0 && HeII_column_density == 0.0)
			...
			{
			  S1 = pow(10.0, interpol_1d_fltdbl(chimesRateTables.NonEqIon->NonEqRates[i].shieldFactor1D[l][0][j], NHI_index, dNHI));
			  S2 = pow(10.0, interpol_2d_fltdbl(chimesRateTables.NonEqIon->NonEqRates[i].shieldFactor2D[l][0][j], NH_eff_index, NHeI_index, dNH_eff, dNHeI));
			  S3 = pow(10.0, interpol_2d_fltdbl(chimesRateTables.NonEqIon->NonEqRates[i].shieldFactor2D[l][1][j], NH_eff_index, NHe_eff_index, dNH_eff, dNHe_eff));
			}
...
void update_T_dependent_rates(struct gasVariables *myGasVars, struct globalVariables *myGlobalVars, struct All_rate_variables_structure *this_all_rates)
{
  ...
  /* T_index for additional rates. */
  get_index_1d_mydbl(chimesRateTables.RatesTables->Temperatures, chimesRateTables.RatesTables->N_Temperatures, log10(temperature), &T_index, &dT);
  ...
  get_index_1d_mydbl(chimesRateTables.NonEqIon->Temperatures, chimesRateTables.NonEqIon->N_Temperatures, log10(temperature), &T_index, &dT);
  for (i = 0; i < chimesRateTables.NonEqIon->N_Elements; i++)
    {
      for (j = 0; j < (chimesRateTables.NonEqIon->N_Ions[i] - 1); j++)
	{
	  this_all_rates->BensRates[i].CollisIon[j] = pow(10.0, interpol_1d_mydbl(chimesRateTables.NonEqIon->NonEqRates[i].betacoll[j], T_index, dT)) * nH_tot;
	  this_all_rates->BensRates[i].Recomb[j] = (pow(10.0, interpol_1d_mydbl(chimesRateTables.NonEqIon->NonEqRates[i].alpharad[j + 1], T_index, dT)) + pow(10.0, interpol_1d_mydbl(chimesRateTables.NonEqIon->NonEqRates[i].alphadi[j + 1], T_index, dT))) * nH_tot;

	  if (chimesRateTables.NonEqIon->NonEqRates[i].CTHrec_mask[j + 1] == 1)
	    this_all_rates->BensRates[i].CTHrec[j] = pow(10.0, interpol_1d_mydbl(chimesRateTables.NonEqIon->NonEqRates[i].CTHrecof[j + 1], T_index, dT)) * nH_tot;
	  if (chimesRateTables.NonEqIon->NonEqRates[i].CTHerec_mask[j + 1] == 1)
	    this_all_rates->BensRates[i].CTHerec[j] = pow(10.0, interpol_1d_mydbl(chimesRateTables.NonEqIon->NonEqRates[i].CTHerecof[j + 1], T_index, dT)) * nH_tot;
	  if (chimesRateTables.NonEqIon->NonEqRates[i].CTHion_mask[j] == 1)
	    this_all_rates->BensRates[i].CTHion[j] = pow(10.0, interpol_1d_mydbl(chimesRateTables.NonEqIon->NonEqRates[i].CTHionof[j], T_index, dT)) * nH_tot;
	  if (chimesRateTables.NonEqIon->NonEqRates[i].CTHeion_mask[j] == 1)
	    this_all_rates->BensRates[i].CTHeion[j] = pow(10.0, interpol_1d_mydbl(chimesRateTables.NonEqIon->NonEqRates[i].CTHeionof[j], T_index, dT)) * nH_tot;
	}
    }  
}

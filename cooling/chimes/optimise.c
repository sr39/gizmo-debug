...
double determine_subcyclestep(struct gasVariables *myGasVars, struct globalVariables *myGlobalVars, int *abundant_indices, struct Species_Structure *species, struct Reactions_Structure *reactions_root, struct All_rate_variables_structure *this_all_rates)
{
  ...
  double HI_column_density, HeI_column_density, HeII_column_density, H2_column_density, CO_column_density, H2O_column_density, OH_column_density, extinction;
  int i;
  ...
  update_species_fluxes(myGasVars->abundances, myGasVars->temperature, reactions_root, species, myGlobalVars->T_mol, 1, myGlobalVars->speciesIndices, myGlobalVars);
  ...
  for (i = 0; i < 9; i++)
    {
	  if (abundant_indices[i] != -1)
		{
		  dt_temp = myGlobalVars->time_tolerance * fabs(myGasVars->abundances[myGlobalVars->speciesIndices[abundant_indices[i]]] / (species[myGlobalVars->speciesIndices[abundant_indices[i]]].creation_rate - species[myGlobalVars->speciesIndices[abundant_indices[i]]].destruction_rate));
		  if (dt_temp < dt_new)
			dt_new = dt_temp;
		}
    }

...
Note that if you wish to experiment with it, enabling `CRK_FACES` will replace the usual kernel function (defined as usual with `KERNEL_FUNCTION`) with the tensor-corrected consistent reproducing kernel as defined in Frontiere, Raskin, and Owen (arXiv:1605.00725). This is currently valid for MFM/MFV. This can give more accurate 'closure' relations for the effective faces, which may reduce certain errors (reducing numerical diffusion in some multi-phase boundaries, and most obviously reducing div-B errors in MHD). However it remains in testing/experimental stages right now, so please experiment for yourself.

...
{
    double dp[3],dv[3],dm_dv,r,dm_vrel;
    double wk, dwk;
    double hinv, hinv3, hinv4;
    double dm_mj_wk, mj_dwk_r;
};
...
void out2particle_dm_density(struct dm_densdata_out *out, int i, int mode)
{
    ...
    if(P[i].Type == 0)
    {
        ...
        ASSIGN_ADD(SphP[i].dm_vrel, out->dm_vrel, mode);
        ASSIGN_ADD(SphP[i].dm_count, out->dm_count, mode);
        ASSIGN_ADD(SphP[i].vx_dm, out->vx_dm, mode);   
        ASSIGN_ADD(SphP[i].vy_dm, out->vy_dm, mode);
        ASSIGN_ADD(SphP[i].vz_dm, out->vz_dm, mode);
...
#pragma omp parallel
            ...
            report_memory_usage(&HighMark_sphdensity, "dm_SPH_DENSITY");
                     ...
                     int *ngblist)
{
    ...
    double r_min = All.BoxSize, ID_min = 0, v_dm = 0, v_b = 0, count = 0;
    ...
    while(startnode >= 0)
    {
        while(startnode >= 0)
        {
            numngb_inbox = ngb_treefind_variable_threads_targeted(local.Pos, local.dm_Hsml , target, &startnode,
                                         mode,  exportflag,  exportnodecount,  exportindex, ngblist, 2);
                                                                                                                         /*i番目のガス粒子周辺のダークマター粒子数*/
           // numngb_inbox = dm_ngb_treefind_variable_threads(local.Pos, local.dm_Hsml, target, &startnode,
            //                                 mode, exportflag, exportnodecount, exportindex, ngblist);
...
//                if(r2 < h2)
//                {
//                out.vx_dm += P[j].Vel[0] / All.cf_atime; 
//                out.vy_dm += P[j].Vel[1] / All.cf_atime;
//                out.vz_dm += P[j].Vel[2] / All.cf_atime;
//                out.dm_count += 1;
//                }
//              }
...
//                kernel.dm_vrel = sqrt(kernel.dv[0] * kernel.dv[0] + kernel.dv[1] * kernel.dv[1] + kernel.dv[2] * kernel.dv[2]); /*ダークマターとガスの相対速度の速度分散*/
        
                double LENGTH_physical = All.UnitLength_in_cm * All.cf_atime / All.HubbleParam; 
//                double sigma_alldm_proton = (CROSS_SECTION_XP / LENGTH_physical / LENGTH_physical) * pow(kernel.dm_vrel / REF_VELOCITY_XP, P_XP-4);/*ダークマターとプロトンの２粒子における散乱断面積*/
                
                    kernel.r = sqrt(r2);
                    u = kernel.r * kernel.hinv;
                    kernel_main(u, kernel.hinv3, kernel.hinv4, &kernel.wk, &kernel.dwk, 0);
                    N_j = P[j].Mass * All.UnitMass_in_g / All.HubbleParam / DARKMATTERMASS;/*ダークマター粒子の個数*/
                    N_i = P[target].Mass * All.UnitMass_in_g / All.HubbleParam / PROTONMASS;/*プロトンの個数*/
                    mass_j = P[j].Mass;
                    ...
                    out.dm_coll += N_i * kernel.wk;
//                    out.dm_vrel += kernel.dm_vrel;

...
static inline double h5(double fi[36], double w0t, double w1t, double w2t, double w0mt, double w1mt,
			double w2mt, double w0d, double w1d, double w2d, double w0md, double w1md,
			double w2md)
{
  return fi[0] * w0d * w0t + fi[1] * w0md * w0t + fi[2] * w0d * w0mt + fi[3] * w0md * w0mt +
    ...
    fi[29] * w1md * w2t + fi[30] * w1d * w2mt + fi[31] * w1md * w2mt + fi[32] * w2d * w2t +
...
static inline double h3(double fi[16], double w0t, double w1t, double w0mt, double w1mt, double w0d,
			double w1d, double w0md, double w1md)
{
  return fi[0] * w0d * w0t + fi[1] * w0md * w0t + fi[2] * w0d * w0mt + fi[3] * w0md * w0mt +
    fi[4] * w0d * w1t + fi[5] * w0md * w1t + fi[6] * w0d * w1mt + fi[7] * w0md * w1mt + fi[8] * w1d * w0t +
    fi[9] * w1md * w0t + fi[10] * w1d * w0mt + fi[11] * w1md * w0mt + fi[12] * w1d * w1t +
    fi[13] * w1md * w1t + fi[14] * w1d * w1mt + fi[15] * w1md * w1mt;
}
...
// radiation
static int
  __attribute__ ((unused)) helm_eos_rad(double rho, double temp, double abar, double zbar, double prad[5],
					double erad[5], double srad[5])
{
  const double rhoi = 1.0 / rho, tempi = 1.0 / temp;
  // prad
  prad[0] =
    4.0 / 3.0 * GSL_CONST_CGS_STEFAN_BOLTZMANN_CONSTANT / GSL_CONST_CGS_SPEED_OF_LIGHT * gsl_pow_4(temp);
  // dprad dd
  prad[1] = 0.0;
  // dprad dt
...
// ion section
static int
  __attribute__ ((unused)) helm_eos_ion(double rho, double temp, double ltemp,
					const struct helm_eos_cache *cache, double abar, double zbar,
					double pion[5], double eion[5], double sion[5])
{
  ...
  // dsion dd
  sion[1] =
    (pion[1] / rho - pion[0] / (rho * rho) + eion[1]) / temp -
    GSL_CONST_CGS_BOLTZMANN * GSL_CONST_NUM_AVOGADRO * ytot / rho;
  // dsion dt
  sion[2] =
    (pion[2] / rho + eion[2]) / temp - (pion[0] / rho + eion[0]) / (temp * temp) +
    1.5 * GSL_CONST_CGS_BOLTZMANN * GSL_CONST_NUM_AVOGADRO * ytot / temp;
  // dsion da
  sion[3] =
    (pion[3] / rho + eion[3]) / temp + GSL_CONST_CGS_BOLTZMANN * GSL_CONST_NUM_AVOGADRO * ytot * ytot * (2.5 -
													 y);
  // dsion dz
  sion[4] = 0.0;
...
static int
  __attribute__ ((unused)) helm_eos_ele(double rho, double temp, double l10temp, double pele[5],
					double eele[5], double sele[5], double etaele[5], double xne[5],
					const struct helm_eos_cache *cache, const int only_e)
{
  ...
  if(iat < 0 || iat >= helm_eos_table->nrho)
    {
      fprintf(stderr, "density (%g) off table [%d:%d], %g %g %g %g %d\n", rho, 0, helm_eos_table->nrho,
	      cache->abar, cache->zbar, cache->ldin, cache->ye, iat);
      return -1;
    }
  ...
  if(!only_e)
    {
      ...
      df_dd =
	h5(fi, si0t, si1t, si2t, si0mt, si1mt, si2mt, ddsi0d, ddsi1d, ddsi2d, ddsi0md, ddsi1md, ddsi2md);

      // derivative with respect to temperature and density
      df_dt =
	h5(fi, dsi0t, dsi1t, dsi2t, dsi0mt, dsi1mt, dsi2mt, dsi0d, dsi1d, dsi2d, dsi0md, dsi1md, dsi2md);
...
// Coulomb corrections
static int
  __attribute__ ((unused)) helm_eos_coul(double rho, double temp, double abar, double zbar, double pcoul[5],
					 double ecoul[5], double scoul[5])
{
  // fitting parameters
  const double
    a1 = -0.898004,
    b1 = 0.96786, c1 = 0.220703, d1 = -0.86097, e1 = 2.5269, a2 = 0.29561, b2 = 1.9885, c2 = 0.288675;
  ...
  if(plasg >= 1.0)
    {
      ...
      y =
	GSL_CONST_NUM_AVOGADRO * GSL_CONST_CGS_BOLTZMANN * temp * ytot * (a1 +
									  0.25 / plasg * (b1 * x - c1 / x));
      ecoul[1] = y * plasgdd;
      ...
      y =
	-GSL_CONST_NUM_AVOGADRO * GSL_CONST_CGS_BOLTZMANN / (abar / plasg) * (0.75 * b1 * x + 1.25 * c1 / x +
									      d1);
      scoul[1] = y * plasgdd;
      scoul[2] = y * plasgdt;
  ...
  else
    {
      ...
      scoul[0] =
	-GSL_CONST_NUM_AVOGADRO * GSL_CONST_CGS_BOLTZMANN / abar * (c2 * x - a2 * (b2 - 1.0) / b2 * y);
      ...
      s =
	-GSL_CONST_NUM_AVOGADRO * GSL_CONST_CGS_BOLTZMANN / (abar * plasg) * (1.5 * c2 * x -
									      a2 * (b2 - 1.0) * y);
      scoul[1] = s * plasgdd;
      scoul[2] = s * plasgdt;
...
int eos_calc_tgiven_azbar(double rho, const struct helm_eos_cache *cache, double temp, struct eos_result *res,
			  const int only_e)
{
  ...
  const size_t offsets[5] =
    { offsetof(struct eos_value, v), offsetof(struct eos_value, drho), offsetof(struct eos_value, dtemp),
offsetof(struct eos_value, dabar), offsetof(struct eos_value, dzbar) };
  ...
  if(res != NULL && !only_e)
	...
	for(i = 0; i < 5; i++)
      ...
      res->sound =
	GSL_CONST_CGS_SPEED_OF_LIGHT * sqrt(res->gamma_1 /
					    (1.0 +
					     (res->e.v +
					      gsl_pow_2(GSL_CONST_CGS_SPEED_OF_LIGHT)) * rho / res->p.v));
...
int helm_eos_update_cache(double rho, double abar, double zbar, struct helm_eos_cache *cache)
{
  ...
  cache->lswot15 =
    1.5 * log((2.0 * M_PI * GSL_CONST_CGS_UNIFIED_ATOMIC_MASS * GSL_CONST_CGS_BOLTZMANN) /

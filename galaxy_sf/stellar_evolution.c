...
double evaluate_light_to_mass_ratio(double stellar_age_in_gyr, int i)
{
    ...
    return calculate_individual_stellar_luminosity(0, m0, i) / m0 * (UNIT_LUM_TO_SOLAR) / (UNIT_MASS_TO_SOLAR);
...
double calculate_individual_stellar_luminosity(double mdot, double mass, long i)
{
    ...
    return P[i].StarLuminosity_Solar / UNIT_LUM_TO_SOLAR;
#endif
    /* if above flags not defined, estimate accretion + main-sequence luminosity as simply as possible */
    double lum=0, lum_sol=0, c_code = C_LIGHT_CODE, m_solar = mass * UNIT_MASS_TO_SOLAR;
...
double particle_ionizing_luminosity_in_cgs(long i)
{
    ...
    double l_sol=bh_lum_bol(0,P[i].Mass,i)*(UNIT_LUM_TO_SOLAR), m_sol=P[i].Mass*UNIT_MASS_TO_SOLAR, r_sol=pow(m_sol,0.738); // L/Lsun, M/Msun, R/Rsun
    ...
    return fion * (l_sol / m_sol) * (1.95*P[i].Mass*All.UnitMass_in_g/All.HubbleParam); // just needs to be multiplied by the actual stellar luminosity to get luminosity to mass ratio
    ...
    if(P[i].Type == 5) {lm_ssp = 1.741e6 * bh_lum_bol(P[i].BH_Mdot,P[i].Mass,i) / (P[i].Mass*UNIT_TIME_TO_MYR*C_LIGHT_CODE*C_LIGHT_CODE);}
#endif
...
void particle2in_addFB_fromstars(struct addFB_evaluate_data_in_ *in, int i, int fb_loop_iteration)
{
    ...
    in->Msne = P[i].SNe_ThisTimeStep * (14.8/UNIT_MASS_TO_SOLAR); // assume every SNe carries 14.8 solar masses (IMF-average)
    in->SNe_v_ejecta = 2.607e8 / All.UnitVelocity_in_cm_per_s; // assume ejecta are ~2607 km/s [KE=1e51 erg, for M=14.8 Msun], which is IMF-averaged
    ...
    in->SNe_v_ejecta = sqrt(2.*(1.e51/All.UnitEnergy_in_cgs)/P[i].Mass); // for SNe [total return], simple v=sqrt(2E/m)should be fine without relativistic corrections
    if(P[i].SNe_ThisTimeStep<1) {double m_msun=P[i].Mass*UNIT_MASS_TO_SOLAR; in->SNe_v_ejecta = (616.e5 * sqrt((1.+0.1125*m_msun)/(1.+0.0125*m_msun)) * pow(m_msun,0.131)) / All.UnitVelocity_in_cm_per_s;} // scaling from size-mass relation+eddington factor, assuming line-driven winds //
#endif
#ifdef METALS
...
double mechanical_fb_calculate_eventrates(int i, double dt)
{
    ...
    double m_sol=P[i].Mass*UNIT_MASS_TO_SOLAR, l_sol=bh_lum_bol(0,P[i].Mass,i)*UNIT_LUM_TO_SOLAR;
    ...
    P[i].SNe_ThisTimeStep = DMIN(0.5, (2.338 * alpha * pow(l_sol,7./8.) * pow(m_sol,0.1845) * (1./q0) * pow(q0*k0,1./alpha) / m_sol) * (dt*UNIT_TIME_TO_GYR)); // Castor, Abbot, & Klein scaling
#endif
    ...
    P[i].SNe_ThisTimeStep = P[i].Mass*UNIT_MASS_TO_SOLAR / 91.; // 1 event per 91 solar masses
    return 1;
#endif
    ...
    if(star_age < 0.03)
    {
        ...
        double p = RSNe * (P[i].Mass*UNIT_MASS_TO_SOLAR) * (dt*UNIT_TIME_TO_MYR); // unit conversion factor
...
void mechanical_fb_calculate_eventrates_Rprocess(int i, double dt)
{
    ...
    {
        double p = 3.0e-5 / (1000.*star_age) * (P[i].Mass*UNIT_MASS_TO_SOLAR) * (dt*UNIT_TIME_TO_MYR); // Nsne/Myr for a 1 Msun population
...
void mechanical_fb_calculate_eventrates_Winds(int i, double dt)
{
    ...
    double m_sol=P[i].Mass*UNIT_MASS_TO_SOLAR, l_sol=bh_lum_bol(0,P[i].Mass,i)*UNIT_LUM_TO_SOLAR; /* luminosity in solar */
    double gam=DMIN(0.5,3.2e-5*l_sol/m_sol), alpha=0.5+0.4/(1.+16./m_sol), q0=(1.-alpha)*gam/(1.-gam), k0=1./30.; // Eddington factor (~L/Ledd for winds), capped at 1/2 for sanity reasons, approximate scaling for alpha factor with stellar type (weak effect)
    p = (2.338 * alpha * pow(L_sol,7./8.) * pow(M_sol,0.1845) * (1./q0) * pow(q0*k0,1./alpha) / m_sol) * (dt*UNIT_TIME_TO_GYR); // mdot in M_sun/Gyr, times dt
#endif
    p=1.-exp(-p);
    ...
    p *= All.StellarMassLoss_Rate_Renormalization * (dt*UNIT_TIME_TO_GYR); // fraction of particle mass expected to return in the timestep //
...
void particle2in_addFB_Rprocess(struct addFB_evaluate_data_in_ *in, int i)
{
    ...
    in->Msne = 0.01 * (double)P[i].RProcessEvent_ThisTimeStep / ((double)(UNIT_MASS_TO_SOLAR)); // mass ejected ~0.01*M_sun; only here for bookkeeping //
#endif
}
...
#endif
    in->Msne = P[i].SNe_ThisTimeStep * (Msne/UNIT_MASS_TO_SOLAR); // total mass in code units
...
void particle2in_addFB_winds(struct addFB_evaluate_data_in_ *in, int i)
{
    ...
    double m_msun=P[i].Mass*UNIT_MASS_TO_SOLAR;
...
double singlestar_subgrid_protostellar_evolution_update_track(int n, double dm, double dt)
{
    ...
    double lum_sol = 0.0, m_initial = DMAX(1.e-37 , (BPP(n).BH_Mass - dm)), mu = DMAX(0, dm/m_initial), m_solar = BPP(n).BH_Mass*UNIT_MASS_TO_SOLAR, T4000_4 = pow(m_solar, 0.55); // m_initial = mass before the accretion, mu = relative mass accreted, m_solar = mass in solar units, T4000_4 = (temperature/4000K)^4 along Hayashi track
    ...
    {// currently on Henyey track, contracting at constant Luminosity
        t_R_evol = 1.815e7 * m_solar*m_solar / (BPP(n).ProtoStellarRadius_inSolar * lum_sol) / (UNIT_TIME_TO_YR); // contraction timescale
        contraction_factor = 1. / (1. + dt/t_R_evol);
    } else {// currently on Hayashi track, contracting at constant Temperature
        t_R_evol = 8.021e7 * m_solar*m_solar / (BPP(n).ProtoStellarRadius_inSolar*BPP(n).ProtoStellarRadius_inSolar*BPP(n).ProtoStellarRadius_inSolar * T4000_4) / (UNIT_TIME_TO_YR); // contraction timescale
    ...
    double mdot_m_solar_per_year = mdot * UNIT_MASS_TO_SOLAR / UNIT_TIME_TO_YR; // accretion rate in msolar/yr
    double m_solar = mass * UNIT_MASS_TO_SOLAR); // mass in units of Msun
    ...
    double r = BPP(n).ProtoStellarRadius_inSolar / UNIT_LENGTH_TO_SOLAR; // star radius in code units
        ...
        if (stage >= 1){ //We only evolve those that are beyond the pre-collapse phase
            ...
            //Note: this subcycling does not update stage, so a protostar could overshoot
                ...
                if (stage==2){ //burning at fixed Tc, lum_D set to keep the central temperature constant
                    ...
                    dm_D = dm_curr - dt_curr * (lum_D*UNIT_LUM_TO_SOLAR/15.) * (1e-5) / (UNIT_MASS_TO_SOLAR/UNIT_TIME_TO_YR) ;
                }
                else{ if (stage>2){
                    //burning all accreted D for stages above 2
                    lum_D = (15./UNIT_LUM_TO_SOLAR) * (mdot_m_solar_per_year/(1e-5));
                ...
                if (fabs(rel_dr) > max_rel_dr){
                    ...
                    r = BPP(n).ProtoStellarRadius_inSolar / UNIT_LENGTH_TO_SOLAR;
                }
                else{
                    loop_subcycle++;
                    r *= (1.0+rel_dr);
            ...
            BPP(n).ProtoStellarRadius_inSolar = r * UNIT_LENGTH_TO_SOLAR;
            BPP(n).Mass_D = mass_D;
            //Debug message
#ifdef PS_EVOL_OUTPUT_MOREINFO
            if (n_subcycle>1){
                dm_D = mass_D - BPP(n).Mass_D; //get the tota change in D mass in the protostar
                rel_dr = r/(BPP(n).ProtoStellarRadius_inSolar/UNIT_LENGTH_TO_SOLAR)-1.0; //get the actual relative change over the whole timestep
            }
            printf("PS evolution t: %g sink ID: %u mass: %g radius_solar: %g stage: %d mdot_m_solar_per_year: %g mD: %g rel_dr: %g dm: %g dm_D: %g Tc: %g Pc: %g rhoc: %g beta: %g dt: %g n_ad: %g lum_int: %g lum_I: %g lum_D: %g age_Myr: %g StarLuminosity_Solar: %g BH_Mass_AlphaDisk: %g SinkRadius: %g dlogbeta_dlogm: %g n_subcycle: %d.ZAMS_Mass %g PS_end\n",All.Time, P[n].ID,m_solar,BPP(n).ProtoStellarRadius_inSolar,stage, mdot_m_solar_per_year, BPP(n).Mass_D*UNIT_MASS_TO_SOLAR,rel_dr,dm*UNIT_MASS_TO_SOLAR, dm_D*UNIT_MASS_TO_SOLAR, Tc, Pc*All.UnitPressure_in_cgs, rhoc* All.UnitDensity_in_cgs, beta, dt*UNIT_TIME_TO_MYR, n_ad, lum_int*UNIT_LUM_TO_SOLAR, lum_I*UNIT_LUM_TO_SOLAR, lum_D*UNIT_LUM_TO_SOLAR, (All.Time-P[n].ProtoStellarAge)*UNIT_TIME_TO_MYR, BPP(n).StarLuminosity_Solar, BPP(n).BH_Mass_AlphaDisk, BPP(n).SinkRadius, dlogbeta_dlogm, n_subcycle, P[n].ZAMS_Mass );
#endif
            ...
            if ( (stage==1) && (Tc >= 1.5e6) && ((All.Time-BPP(n).StellarAge) > DMAX(3.*dt, 1e-4/(UNIT_TIME_TO_MYR)) ) ){ //further check that the sink has been promoted at least a couple of timesteps and 100 yr ago, so that we don't start D burning immediately after forming the sink (relevant in low res cases)
    ...
    //BPP(n).StarLuminosity_Solar = (lum_acc + lum_disk + lum_int) * UNIT_LUM_TO_SOLAR ; //luminosity of the star
    /*********************************************/
    ...
    BPP(n).StarLuminosity_Solar = (eps_protostar*All.G*mass*mdot/r + lum_int) * UNIT_LUM_TO_SOLAR; //luminosity of the star in solar units
#else
    BPP(n).StarLuminosity_Solar = (eps_protostar*All.G*mass*mdot/r + lum_Hayashi) * UNIT_LUM_TO_SOLAR; //same as above but we don't count H burning for th emission. Thsi way the radial evolution follows the same track as with the full model, but we don't provide feedback from H burning to the nearby gas
#endif
...
#endif
    {printf("PS evolution t: %g sink ID: %u mass: %g radius_solar: %g stage: %d mdot_m_solar_per_year: %g mD: 0 rel_dr: 0 dm: %g dm_D: 0 Tc: 0 Pc: 0 rhoc: 0 beta: 0 dt: %g n_ad: 0 lum_int: 0 lum_I: 0 lum_D: 0 age_Myr: %g StarLuminosity_Solar: %g BH_Mass_AlphaDisk: %g SinkRadius: %g dlogbeta_dlogm: 0 n_subcycle: 0.ZAMS_Mass %g PS_end\n",All.Time, P[n].ID,BPP(n).BH_Mass*UNIT_MASS_TO_SOLAR,BPP(n).ProtoStellarRadius_inSolar,BPP(n).ProtoStellarStage, BPP(n).BH_Mdot*UNIT_MASS_TO_SOLAR/UNIT_TIME_TO_YR , dm*UNIT_MASS_TO_SOLAR, dt*UNIT_TIME_TO_MYR, (All.Time-P[n].ProtoStellarAge)*UNIT_TIME_TO_MYR, BPP(n).StarLuminosity_Solar, BPP(n).BH_Mass_AlphaDisk*UNIT_MASS_TO_SOLAR, BPP(n).SinkRadius, P[n].ZAMS_Mass );}
#endif
}
...
double single_star_wind_mdot(int n, int set_mode){ //if set_mode is zero then the wind mode is not changed by calling this function
    ...
    double m_solar = BPP(n).Mass * UNIT_MASS_TO_SOLAR; // mass in units of Msun
    ...
    wind_mass_loss_rate = pow(10.0,logmdot_wind) / (UNIT_MASS_TO_SOLAR/UNIT_TIME_TO_YR); //reducing the rate to be more in line with observations, see Nathan Smith 2014, conversion to code units from Msun/yr
...
/* Let's get the wind velocity for MS stars */
    ...
    double v_esc = 6.177e7 / All.UnitVelocity_in_cm_per_s * sqrt(P[n].Mass / P[n].ProtoStellarRadius_inSolar); // surface escape velocity - wind escape velocity should be O(1) factor times this, factor given below
...
double ps_beta(double m, double n_ad, double rhoc, double Pc) {
    double mass=m*UNIT_MASS_TO_SOLAR;//in units of solar mass
...
double ps_lum_I(double mdot) {
    double mdot_m_solar_per_year = mdot * UNIT_MASS_TO_SOLAR/UNIT_TIME_TO_YR; // accretion rate in msolar/yr
    return (2.5/UNIT_LUM_TO_SOLAR) * ( mdot_m_solar_per_year/(1e-5));
}
...
double ps_lum_Hayashi_BB(double m, double r) {
    double m_solar = m * UNIT_MASS_TO_SOLAR, T4000_4 = pow(m_solar , 0.55), r_solar = r * UNIT_LENGTH_TO_SOLAR; // protostellar temperature along Hayashi track
    T4000_4 = 0.316406; //ORION prescription T=3000K
    return 0.2263 * r_solar * r_solar * T4000_4 / UNIT_LUM_TO_SOLAR; // luminosity from KH contraction
}
...
double ps_lum_MS(double m) {
    double m_solar = m * UNIT_MASS_TO_SOLAR;
    if(m_solar <= 0.1) {return 0;} // zero luminosity below some threshold for fits, otherwise use formula below //
    return ((0.39704170*pow(m_solar,5.5) + 8.52762600*pow(m_solar,11)) / (0.00025546+pow(m_solar,3)+5.43288900*pow(m_solar,5) + 5.56357900*pow(m_solar,7) + 0.78866060*pow(m_solar,8)+0.00586685*pow(m_solar,9.5))) / UNIT_LUM_TO_SOLAR;
}

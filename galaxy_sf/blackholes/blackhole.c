...
int bh_check_boundedness(int i, int j, double vrel, double vesc, double dr_code)
{
	...
	major_axis_max = 2*PPP[i].SinkRadius; //DMAX(2*PPP[i].SinkRadius, Get_Particle_Size(j)); //All.ForceSoftening[5];
#else
 ...
 */
    ...
    if(BPP(n).BH_Mass_AlphaDisk > 0)
    {
        ...
        //mdot = All.BlackHoleAccretionFactor * BPP(n).BH_Mass_AlphaDisk / (1.0e5 * t_yr) * pow(BPP(n).BH_Mass_AlphaDisk/(BPP(n).BH_Mass_AlphaDisk+BPP(n).BH_Mass), 0.4);
#endif

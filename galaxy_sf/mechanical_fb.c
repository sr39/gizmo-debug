...
#pragma omp parallel
        ...
        if(fb_loop_iteration== 3) {TAG_TO_USE = TAG_FBLOOP_6A;}
...
#pragma omp parallel
        ...
        if(fb_loop_iteration== 3) {TAG_TO_USE = TAG_FBLOOP_6B;}
        for(ngrp = 1; ngrp < (1 << PTask); ngrp++)
        {
            recvTask = ThisTask ^ ngrp;
            if(recvTask < NTask)
    ...
    while(startnode >= 0)
    {
        while(startnode >= 0)
        {
            ...
            for(n = 0; n < numngb_inbox; n++)
            {
                ...
                /* AJE make sure normalization is OK here */
                for(k=NUM_METAL_SPECIES-NUM_AGE_TRACERS;k<NUM_METAL_SPECIES;k++){P[j].Metallicity[k] = P[j].Metallicity[k] + local.yields[k];}
#endif

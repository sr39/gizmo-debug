...
void do_sinks(void)
{
...
#ifdef MAGNETIC
	  sink_send[index].rho = SphP[NumPart - NumSinks + i].Density;
#endif
	  index++;
	}
  ...
  for(i = local.nh_max = sink_index = 0; i < N_gas; i++)
	...
	if(All.TotNumSinks == 0 || r2_min > All.SinkHsml * All.SinkHsml)
	  {
	    nh =
	      HYDROGEN_MASSFRAC * SphP[i].Density * All.UnitDensity_in_cgs * a3inv * hubble_param2 /
	      PROTONMASS;
...
#endif
#ifdef MAGNETIC
		  sink_send[index].rho = SphP[i].Density;
#endif

...
#else
      dt = P[i].dt_step * All.Timebase_interval / hubble_a;
#endif
          ...
          if( (P[j].Mass>0) && (SphP[j].Density>0) )
          {
            ...
            r2 += (P[i].Hsml/5.)*(P[i].Hsml/5.); // just a small number to prevent errors on near-overlaps
               ...
               if(dv_imparted_uv > dv_imparted)
               {
                ...
                for(k=0; k<3; k++)
                {
                  P[j].Vel[k] += v * ascale * dir[k];
                  SphP[j].VelPred[k] += v * ascale * dir[k];
                }
...
/* if we're not forcing the kick orientation, need to separately apply the UV kick */
  ...
  myfree(Ngblist);
   ...
   if(ThisTask == 0)
   {
     ...
     printf("Momentum Wind Feedback: Time=%g Nkicked=%g L/c=%g Momkicks=%g V_avg=%g tau_j_mean=%g \n",

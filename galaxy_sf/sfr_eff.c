...
#ifdef SINGLE_STAR_USE_GRADRHO
    if(P[i].min_dist_to_bh < DMAX(press_grad_length,PPP[i].Hsml)) {rateOfSF=0;}
#else
    if(P[i].min_dist_to_bh < PPP[i].Hsml) {rateOfSF=0;} // restrict to particles without a sink in their kernel or within 1 pressure gradient scale length (1 hydrostatic core should spawn 1 sink at the centre)
#endif
#endif

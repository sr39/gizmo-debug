            ...
            if((1<<P[i].Type) & HERMITE_INTEGRATION)
            {
                if(mode == 0)
                {
                    ...
                    P[i].OldJerk[j] = P[i].GravJerk[j];
                     // we used Hermite_OldAcc to store the gravitaional field from before the most recent kick before the first Hermite tree pass, so now we do a switcheroo to restore GravAccel to the proper value to use with the upcoming KDK timestep, to integrate self-consistently with 

...
/* in this case, we have a shearing box with the '1' coordinate being phi, so there is a periodic extra wrap; 
    ...
    dx = NGB_PERIODIC_LONG_X(dx0,current->center[1]-searchcenter[1],current->center[2]-searchcenter[2],-1);
    if(dx > dist) continue;
    dz = NGB_PERIODIC_LONG_Z(dx0,current->center[1]-searchcenter[1],current->center[2]-searchcenter[2],-1);
    if(dz > dist) continue;
    ...
    if((dx_abs + dx_node < boxHalf_X) || (dx_abs - dx_node > boxHalf_X))
    {
        ...
        dy = NGB_PERIODIC_LONG_Y(dx0,current->center[1]-searchcenter[1],current->center[2]-searchcenter[2],-1);
        if(dy > dist) continue;
    } else {
        ...
        MyDouble dy_m = NGB_PERIODIC_LONG_Y(dx0-dx_node,dy0,current->center[2]-searchcenter[2],-1); // one edge of x-side of box
        MyDouble dy_p = NGB_PERIODIC_LONG_Y(dx0+dx_node,dy0,current->center[2]-searchcenter[2],-1); // other edge, should bracket possible wrapping
...
/* this is the 'normal' operation mode */
    ...
    dy = NGB_PERIODIC_LONG_Y(current->center[0] - vcenter.d[0], current->center[1] - vcenter.d[1], current->center[2] - vcenter.d[2],-1);
    dz = NGB_PERIODIC_LONG_Z(current->center[0] - vcenter.d[0], current->center[1] - vcenter.d[1], current->center[2] - vcenter.d[2],-1);
    // now test against the minimal sphere enclosing everything

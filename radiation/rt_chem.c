...
void rt_get_sigma(void)
{
    ...
    {
        e_start = nu[i]; if(i == N_RT_FREQ_BINS - 1) {e_end = 500.0;} else {e_end = nu[i+1];} d_nu = (e_end - e_start) / (float)(integral - 1);
...
void rt_update_chemistry(void)
{
    ...
    double nHII;
    double dtime, c_light;
    double A, B, CC;
    double n_photons_vol;
    ...
    for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
        if(P[i].Type == 0)
        {
            ...
            n_photons_vol = rt_return_photon_number_density(i,0);
            ...
            B = dtime * c_light * n_photons_vol * rt_sigma_HI[0];
...
void rt_update_chemistry(void)
{
    int i, j;
    double nH, temp, molecular_weight, rho;
    double nHII, c_light, n_photons_vol;
    double dtime;
    double A, B, CC;
    double alpha_HII, gamma_HI;
    double fac;
    ...
    for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
        if(P[i].Type == 0)
        {
            ...
            for(j = 0; j < N_RT_FREQ_BINS; j++)
            {
                ...
                if(nu[j] >= 13.6)
                    k_HI += c_light * rt_sigma_HI[j] * n_photons_vol;
                
#ifdef RT_CHEM_PHOTOION_HE
                if(nu[j] >= 24.6)
                    k_HeI += c_light * rt_sigma_HeI[j] * n_photons_vol;
                
                if(nu[j] >= 54.4)
                    k_HeII += c_light * rt_sigma_HeII[j] * n_photons_vol;
#endif
            }

...
void n(void)
{
  ...
  for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
    if(P[i].Type == 0)
      {
	trace = SphP[i].n[0]*SphP[i].n[0] + SphP[i].n[1]*SphP[i].n[1] + SphP[i].n[2]*SphP[i].n[2];
	trace = sqrt(trace);
      ...
      while(no >= 0)
	{
	  if(no < All.MaxPart)	/* single particle */
	      ...
	      if(!(nop->u.d.bitflags & (1 << BITFLAG_MULTIPLEPARTICLES)))
		{
	      ...
	      dx = pos_x - nop->stellar_s[0];
	      dy = pos_y - nop->stellar_s[1];
	      dz = pos_z - nop->stellar_s[2];
	    }

...
void apply_grain_dragforce(void)
{
  ...
  //if(All.DragForceMultiplier<=0) return;
  ...
  if(P[i].Type != 0)
	...
	if(dt > 0)
	{
	//for(k=0; k<3; k++) dvel[k] = P[i].Vel[k] - P[i].Gas_Velocity[k];
	//  dv_mag=0; for(k=0;k<3;k++) dv_mag+=dvel[k]*dvel[k]; dv_mag=sqrt(dv_mag);
    soundspeed = Particle_effective_soundspeed_i(i); // physical units

	/* apply actual drag force */
	R_grain = P[i].Grain_Size; // grain size in --code-- units //
 	
 	// epstein drag stopping time //
 	t_stop = (All.Grain_Internal_Density*R_grain) / (P[i].Gas_Density*soundspeed);
 	slow_fac = exp(-dt/t_stop);
 	delta_egyspec = 0;
 	for(k=0; k<3; k++) {
 	  vel_new = P[i].Gas_Velocity[k] + slow_fac*(P[i].Vel[k]-P[i].Gas_Velocity[k]);

...
#
# A. Emerick - testing for isolated galaxy
#
#
GALSF_FB_FIRE_AGE_TRACERS_CUSTOM
GALSF_FB_FIRE_AGE_TRACERS_SURFACE_YIELDS

# For best test of age tracers, keep surface abundance return ON, but turn
# on testing mode. This allows for surface return of metals in sim, but
# turns off the scaling that accounts for deviations from solar. Then,
# test with Age tracer surface yields on and off (above) to see how well
# we do reproducing a matched model. Then turn off all flags below to see
# how we do reproducing a "real" model

AGE_TRACER_TEST_MODE         # turn this and NO_METAL_DEP_YIELDS (below) ON to do matched test

# NO_SURFACE_ABUNDANCES      # Testing purposes - turns off all surface abundances */
# NO_WIND_YIELDS             # Testing purposes - turns off wind mass loss and yields */
NO_METAL_DEP_YIELDS          # For testing - turns off all metallicity dependence in yields - sets wind rates to solar
...
#COOL_GRACKLE_CHEMISTRY=0       # choose Grackle cooling chemistry: (0)=tabular, (1)=Atomic, (2)=(1)+H2+H2I+H2II, (3)=(2)+DI+DII+HD

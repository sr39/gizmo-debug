/* ---------------------------------------------------------------------------------
 ...
 --------------------------------------------------------------------------------- */
{
    int m; double V_i=local.V_i, V_j=get_particle_volume_ags(j), rho_i=local.Mass/V_i*All.cf_a3inv, rho_j=P[j].Mass/V_j*All.cf_a3inv, Face_Area_Vec[3], Face_Area_Norm=0, psi_i, psi_j, vf0_dot_dp=0; // calculate densities (in physical units)
    ...
    for(k=0;k<3;k++)
    {
        ...
        vf0_dot_dp += 0.5*(local.Vel[k] + P[j].Vel[k]) * kernel.dp[k];
    }
    Face_Area_Norm = sqrt(Face_Area_Norm);
    
    double theta_i[CBE_INTEGRATOR_NBASIS]={0}, theta_j[CBE_INTEGRATOR_NBASIS]={0}, v_wt_sum=0, vface[3]={0};
    for(m=0;m<CBE_INTEGRATOR_NBASIS;m++)
    {
        double vi_dot_dp=0, vj_dot_dp=0;
        for(k=0;k<3;k++)
        {
            vi_dot_dp += local.CBE_basis_moments[m][k+1]*kernel.dp[k];
            vj_dot_dp += P[j].CBE_basis_moments[m][k+1]*kernel.dp[k];
        }
        vi_dot_dp -= vf0_dot_dp; vj_dot_dp -= vf0_dot_dp;
        ...
        v_wt_sum += w0_i*local.CBE_basis_moments[m][0] + w0_j*P[j].CBE_basis_moments[m][0]; // summed weights for interaction
        for(k=0;k<3;k++) {vface[k] += w0_i*local.CBE_basis_moments[m][k+1] + w0_j*P[j].CBE_basis_moments[m][k+1];} // summed velocities
    }
    ...
    if(v_wt_sum > 0)
    {
        ...
        for(m=0;m<CBE_INTEGRATOR_NBASIS;m++)
        {
            ...
            if(theta_i[m] == 1)
            {
                vsig_i = do_cbe_flux_computation(local.CBE_basis_moments[m] , vface_dot_A, Face_Area_Vec, flux); // moments are physical, these are as well //
            ...
            if(theta_j[m] == 1)
            {
                vsig_j = do_cbe_flux_computation(P[j].CBE_basis_moments[m] , vface_dot_A, Face_Area_Vec, flux); // moments are physical, these are as well //
                for(k=0;k<CBE_INTEGRATOR_NMOMENTS;k++)
                {

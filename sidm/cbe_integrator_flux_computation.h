 ...
 --------------------------------------------------------------------------------- */
{
    ...
    if((v_wt_sum > MIN_REAL_NUMBER) && (v_wt_sum < MAX_REAL_NUMBER))
    {
        ...
        for(m=0;m<CBE_INTEGRATOR_NBASIS;m++)
        {
            ...
            if(theta_i[m] == 1)
            {
                vsig_i = do_cbe_flux_computation(local_CBE_basis_moments[m] , vface_dot_A, Face_Area_Vec, flux); // moments are physical, these are as well //
                for(k=0;k<CBE_INTEGRATOR_NMOMENTS;k++)
                {
            ...
            if(theta_j[m] == 1)
            {
                vsig_j = do_cbe_flux_computation(Pj_CBE_basis_moments[m] , vface_dot_A, Face_Area_Vec, flux); // moments are physical, these are as well //
                for(k=0;k<CBE_INTEGRATOR_NMOMENTS;k++)
                {
